...
if (isset($_GET['status']) AND isset($_GET['amount']) AND isset($_GET['oid'])) {
    ...
    $paymentMethod = json_decode($_GET['amount']);
    ...
    if ($payst == "paid") {
       // echo $payst.$orderID;
        ...
        //echo $cms->LbaseUrl('user_dashboard/download-ticket_in_dirMovie.php?order_id='. $orderID . "&order_status=$payst");
        //exit();
        $EmailBody = file_get_contents($cms->LbaseUrl('email/body/order_movie_ticket.php?order_id=' . $orderID . "&order_status=$payst"));
         $attachmentpdfcore = file_get_contents($cms->LbaseUrl('user_dashboard/download-ticket_in_dirMovie.php?order_id='. $orderID . "&order_status=$payst"));
