...
foreach ($param as $par):
    ...
    if ($chk == 0) {
        $sql = "INSERT INTO eventwise_payment_method SET event_id='$event_id',payment_method_id='$par->check_name_ID',status='$par->check_namest',date='" . date('Y-m-d') . "',offline_status='1'";
        ...
        $sql = "UPDATE eventwise_payment_method SET status='$par->check_namest',date='" . date('Y-m-d') . "',offline_status='1' WHERE event_id='$event_id' AND payment_method_id='$par->check_name_ID'";
        $result = mysqli_query($con, $sql);
        if ($result == 1) {
            $i+=1;
