...
//    $eventDetails = $data->event;
        print_r($eventDetails = $data->event);
...
//    print_r($ticketDetails = $data->ticket);
...
//    print_r($paymentMethod = $data->paymentMethod);
...
//INSERT BUTTON LEBEL DATA
    ...
    if (!empty($coverI)) {
        ...
        $es = GenerateImageMove($coverI, Homepage_Top_Banner, "ES", HTBW, HTBH);
        $het = GenerateImageMove($coverI, Homepage_Event_Thumbs, "HET", HETW, HETH);
        $etb = GenerateImageMove($coverI, Event_Top_Banner, "ETB", ETBW, ETBH);
        $elt = GenerateImageMove($coverI, Event_List_Thumbs, "ELT", ELTW, ELTH);
        ...
        mysqli_query($con, "UPDATE events SET event_web_banner='$coverIMG' WHERE event_id='$event_id'");
    ...
    if (!empty($thumbI)) {
       // echo 'img 2';
        $es_card = GenerateImageMove($thumbI, Homepage_Top_Banner, "ES_CARD", HTBW, HTBH);
        $het_card = GenerateImageMove($thumbI, Homepage_Event_Thumbs, "HET_CARD", HETW, HETH);
        $etb_card = GenerateImageMove($thumbI, Event_Top_Banner, "ETB_CARD", ETBW, ETBH);
        $elt_card = GenerateImageMove($thumbI, Event_List_Thumbs, "ELT_CARD", ELTW, ELTH);
