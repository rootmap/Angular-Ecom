        ...
        .controller('createEventController', function ($scope, $http, $timeout, growl, listEdit) {
            ...
            $scope.addToList = function () {
                ...
                if ($scope.c < 3) {
                    ...
                    growl.warning("You can add maximum 3 tags", {title: ' '});
                }
...
//                console.log($scope.StartDate,$scope.EndDate,$scope.StartTime,$scope.EndTime);
                ...
                if ($scope.imagecover != null && $scope.imagethumble != null)//Image uploaded condition stsrt here
                {
                    ...
                    growl.info("Creating event please wait...", {title: ' '});
                        ...
                        'cover': $scope.imagecover, 'card': $scope.imagethumble})
                            .success(function (data, status, heards, config) {
                                ...
                                if (data == 1)
                                {
                                    ...
                                    $http.post('./email/merchentTicketCreation.php', {'uri': addEvent.EventURL}).success(function (data, status, heards, config) {
                                        console.log('mail send');
                                        growl.success("Thank You, Your event is created successfully.", {title: ' '});
...
//                });
//
//                console.log( $scope.tags2.length);
//                if ($scope.tags2.length !=3) {
//                    $scope.tags.push({
//                        'tag': tag
//                    });
...
//            }
            ...
            $scope.defaultImageCover = function (img) {
                ...
                if (img != '') {
                    $('.previewOne').hide();
                    $scope.previewOneDefault = true;
                    growl.info("Cover image selected", {title: ' '});
            ...
            $scope.defaultImageThumb = function (img) {
                ...
                if (img != '') {
                    $('.previewTwo').hide();
                    $scope.previewTwoDefault = true;
                    growl.info("Thumbnail image selected", {title: ' '});
            ...
            $scope.myFunction = function () {
                ...
                //x = document.getElementById("evt_desc").value;
                //console.log(x);
            ...
            $scope.createEventJourney = function (evt, status, btn_click)
            {
                ...
                if ($scope.imageOne != null || $scope.imageCover != null) {
                    growl.info("Please wait your event info is processing to create new even", {title: ' '});
                    $timeout(function () {
                        // setTimeout(function () {

                        $http.post('./php/controller/createEventJourneycontroller.php', {'event': evt, 'ticket': $scope.tck, 'tags': $scope.list, imageOne: $scope.imageOne, imageTwo: $scope.imageTwo, paymentMethod: $scope.pmrows, offlinePaymentMethod: $scope.off_pmrows, pick_point: $scope.ppl, coverD_img: $scope.imageCover, thambD_img: $scope.imageThamb, e_status: status, evt_desc: eventD, tick_desc: ticketD, tick_mess_atten: tick_mess_attenD}).success(function (data, status, heards, config) {
                            ...
                            if ($scope.result > 0) {
                                ...
                                if (btn_click == 'btn_preview') {
                                    setTimeout(function () {
                                        window.open(".././event_preview.php?id=" + data, "MsgWindow", "width=1200,height=700");
                                    }, 500);
                                } else {
                                    setTimeout(function () {
            ...
            $scope.imageUploadJourney = function (event) {
                // console.log($scope.previewTwoDefault+'c');
                var files = event.target.files; //FileList object 
                var file = files[files.length - 1];
                $scope.file = file;
                var reader = new FileReader();
            ...
            $scope.imageUpload_thumbleJourney = function (event) {
                //console.log($scope.previewTwoDefault+'t');
                var files = event.target.files; //FileList object 
                var file = files[files.length - 1];
                $scope.file = file;
                var reader = new FileReader();
