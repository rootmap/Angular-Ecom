...
@$pickup_point = json_decode($data->pickup);

 @$pickup_point_name = $pickup_point->name;
 @$pickup_point_add = $pickup_point->address;
 @$pickup_point_details = $pickup_point->point_details;
...
$cemail = $data->userCreate->email;
$cname = $data->userCreate->name;
$cphone = $data->userCreate->phone;
//print_r($customerInfo);
//exit();
$customerEmail = $cemail;
$customerFirstName = $cname;
...
////        //print_r($includedata[$index]);
////       //echo $ticket->quantity+$ticketQuantity_tic."<br>";
////        //echo $includedata[$index]->ticket_id."<br>";
//
...
//
//            if (!empty($includedata[$index]->quantity)) {
//                $ticketQuantity_inc += $includedata[$index]->quantity;
//                //$ticketTotalAmount_inc += $includedata[$index]->price;
//                $ticketTotalAmount_inc += $includedata[$index]->price * $ticketQuantity_inc;
//            }
//        }
//
...
//        $totalAmount .= $ticketTotalAmount_tic + $ticketTotalAmount_inc;
        ...
        if (!empty($totalquantity)) {
            if (!empty($ticketQuantity_tic || !empty($ticketQuantity_free))) {
...
//                    $row = mysqli_fetch_array($sqluser);
                    ...
                    /* $chkuser = mysqli_num_rows($sqluser);
                      if ($chkuser == 0) {
                      $sql = "INSERT INTO users SET  user_first_name='$customerFirstName',user_last_name='$customerLastName', user_email='$customerEmail'";
                    ...
                    //INSERT FUNCTION END HERE
                    //echo "if";
                    //$sqluser = mysqli_query($con, "SELECT user_id FROM guest_user WHERE user_email='$customerEmail' ORDER BY user_id DESC LIMIT 1");
                    //USER ID SHOW HERE
                    // $row = mysqli_fetch_array($sqluser);
                    $user_id = 10;
                }
...
//                    $field_id = $fetchgetfieldid['form_id'];
//
//                    mysqli_query($con, "INSERT INTO event_form_values SET EFV_event_id='$eventId',EFV_session_id='$sessionId',EFV_user_id='$user_id',EFV_field_id='$field_id',EFV_field_value='$value'");
//                    //echo $value . "<br>";
//                }
...
//                    $orderSqlresult = mysqli_query($con, $orderSql);
                    if ($payment_type == 'Bkash Payment') {
                        $orderSql = "INSERT INTO  orders SET order_user_id='$user_id',order_created='$ordercreated',order_read='no',order_number='[" . date('dmy') . "]-" . $loid . "',order_status='booking',order_payment_type='Bkash',order_method='',order_total_item='$totalquantity',order_total_amount='$totalAmount',order_session_id='$sessionId',order_shipment_charge='$deliveryCost'";
                        $orderSqlresult = mysqli_query($con, $orderSql);
                    } elseif ($payment_type == 'Online Payment') {
                        $orderSql = "INSERT INTO  orders SET order_user_id='$user_id',order_created='$ordercreated',order_read='no',order_number='[" . date('dmy') . "]-" . $loid . "',order_status='booking',order_payment_type='eticket',order_method='deliver',order_total_item='$totalquantity',order_total_amount='$totalAmount',order_session_id='$sessionId',order_shipment_charge='$deliveryCost'";
                        $orderSqlresult = mysqli_query($con, $orderSql);
                    } elseif ($payment_type == 'Pay Online & Get E-Ticket on Your E-Mail') {
                        $orderSql = "INSERT INTO  orders SET order_user_id='$user_id',order_created='$ordercreated',order_read='no',order_number='[" . date('dmy') . "]-" . $loid . "',order_status='booking',order_payment_type='eticket',order_method='deliver',order_total_item='$totalquantity',order_total_amount='$totalAmount',order_session_id='$sessionId',order_shipment_charge='$deliveryCost'";
                        $orderSqlresult = mysqli_query($con, $orderSql);
                    } else {
                        $orderSql = "INSERT INTO  orders SET order_user_id='$user_id',order_created='$ordercreated',order_read='no',order_number='[" . date('dmy') . "]-" . $loid . "',order_status='booking',order_payment_type='card',order_method='deliver',order_total_item='$totalquantity',order_total_amount='$totalAmount',order_session_id='$sessionId',order_shipment_charge='$deliveryCost'";
                        $orderSqlresult = mysqli_query($con, $orderSql);
                    }
                    exit();
                    //ORDER INSERT FUNCTION END HERE
                ...
                if (!empty($pickup_point_name)) {
                    $pickup_pointSql = mysqli_query($con, "INSERT INTO `order_pickup_point` SET `user_id`='$o_user_id', `event_id`='$eventId', `order_id`='$sessionId',`name`='$pickup_point_name', `address`='$pickup_point_add', `point_details`='$pickup_point_details'");
                }
                if (!empty($homecity)) {
                    $sqlHD = mysqli_query($con, "INSERT INTO `order_home_delivery` SET event_id='$eventId', order_id='$sessionId', user_id='$o_user_id', address='$homeadd', city='$homecity', country='$homecountry'");
                }
//ORDER INSERT FUNCTION END HERE 
                $ticketUnitPrice = 0;
                foreach ($ticketdata as $index => $ticket) {
...
//                
//        $ticketQuantity_inc= $includedata[$index]->quantity;
//        $ticketUntiPrice_inc= $includedata[$index]->price;
//        $ticket_id_inc= $includedata[$index]->ticket_id;
//        if($ticketType=='3'){
//        $sql_OI=$sql_OI="INSERT INTO order_items SET OI_OE_id='$eventId', OI_order_id='$order_id', OI_item_id='$ticket_id_inc', OI_session_id='$sessionId', OI_unique_id='$unique_id' ,OI_item_type='include',OI_quantity='$ticketQuantity', OI_unit_price='$ticketUnitPrice',OI_created_on='$ordercreated',OI_is_verified='no' ";
