...
$sql1 = "SELECT  
            ...
            WHERE (events.`event_tag` = '$tag' OR events.`event_tag` like '%,$tag,%' OR events.`event_tag` like '$tag,%' OR events.`event_tag` like '%,$tag')
            
            AND events.`event_is_featured`=  'yes '
            AND events.`event_status` = 'active' 
            #OR  events.`event_status` = 'info' OR events.event_id IN (SELECT `status` FROM event_movie_list WHERE `status`='1') 
