...
if ($verb == "GET") {
    ...
    if (isset($_GET['event_id'])) {
        if ($_GET['event_id'] == 0) {
            $nevv = $_GET['event_id'];
            $sqlOrderList = "SELECT evt.* FROM (SELECT orders.order_id,
                   order_events.OE_event_id,
                   orders.order_read,
                   ...
                   (((orders.order_total_amount-orders.order_promotion_discount_amount)-orders.order_discount_amount)+orders.order_shipment_charge) as total,
                   admins.admin_full_name 
                   FROM orders 
                   ...
                   LEFT JOIN events ON `order_events`.OE_event_id = events.event_id) as evt ";

            if (isset($_GET['st'])) {
                $nfrom = clean($_GET['from']);
                $nto = clean($_GET['to']);
                $sqlOrderList .= " WHERE evt.order_created>='$nfrom' AND evt.order_created<='$nto'";
            }

            if ($adminEventPermission == "created") {
                $sqlOrderList .= "WHERE evt.event_created_by=$adminID ";
            } elseif ($adminEventPermission == "selected") {
                if ($adminEventID != "") {
                    $sqlOrderList .= "WHERE evt.event_id IN ($adminEventID) ";
                } else {
                    $sqlOrderList .= "WHERE evt.event_id IN (0) ";
                }
            }
            $sqlOrderList .= " GROUP BY evt.order_id "
                    . "ORDER BY evt.order_read DESC, evt.order_id DESC";
        } else {
            ...
            $sqlOrderList = "SELECT evt.* FROM (SELECT orders.order_id,
                   ...
                   LEFT JOIN admins ON orders.order_updated_by = admins.admin_id 
                   LEFT JOIN events ON `order_events`.OE_event_id = events.event_id) as evt 
WHERE evt.OE_event_id='" . $nevv . "'";

            if (isset($_GET['st'])) {
                $nfrom = clean($_GET['from']);
                $nto = clean($_GET['to']);
                $sqlOrderList .= " AND evt.order_created>='$nfrom' AND evt.order_created<='$nto'";
            }
        ...
        $sqlOrderList = "SELECT orders.order_id,orders.order_read,orders.order_updated_by,order_billing_phone,"
