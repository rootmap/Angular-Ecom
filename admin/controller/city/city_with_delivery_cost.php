...
if ($verb == "GET") {
    $arrayCity = array();
    $sqlCity = "SELECT cities.city_id,cities.country_id,cities.city_name,cities.event_id,events.event_title as event_name,cities.city_delivery_charge,cities.city_status,cities.city_updated,cities.city_updated_by, countries.* FROM event_cities as cities LEFT JOIN countries ON cities.country_id = countries.country_id LEFT JOIN events ON events.event_id=cities.event_id ORDER BY city_id DESC";
...
if ($verb == "POST") {
    ...
    $update_query .=', event_id ="' . $event_id . '"';
    ...
    $run_update_query = "UPDATE event_cities SET $update_query WHERE city_id = $city_id";
...
if ($verb == "PUT") {
    ...
    $event_id = mysqli_real_escape_string($con, $request_vars["event_id"]);
...
if ($verb == "DELETE") {
    ...
    $delete_sql = "DELETE FROM event_cities WHERE city_id = '" . $city_id . "'";
