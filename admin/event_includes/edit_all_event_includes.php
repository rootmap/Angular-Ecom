...
$get_includes_sql = "SELECT event_includes.*, `events`.event_title, event_venues.venue_title FROM event_includes LEFT JOIN `events` ON event_includes.EI_event_id = `events`.event_id LEFT JOIN event_venues ON event_includes.EI_venue_id = event_venues.venue_id WHERE EI_id = '$EI_id'";
$get_result = mysqli_query($con, $get_includes_sql);
$count_includes = mysqli_num_rows($get_result);
if ($count_includes > 0) {
        ...
        $(document).ready(function () {
            $("#btnCreateEventInclude").click(function () {
                ...
                if (EI_event_id === '0') {
                    ...
                    $("#includesError").html('<div class="alert alert-danger fade in"><button class="close" type="button" data-dismiss="alert" aria-hidden="true">×</button><strong><em>Select venue title for includes</em></strong></div>');
                } else if (EI_name === "") {
