...
//    debug($_POST);exit();
    ...
    if ($countQuestion >= 1) {
        ...
        if ($result) {
            $msg = "Event FAQ saved successfully";
            $link = "faq_list.php?msg=" . base64_encode($msg);
            redirect($link);
        } else {
            if (DEBUG) {
...
<html>
    ...
    <body class="">
        ...
        <div id="content"><h3 class="bg-white content-heading border-bottom strong">Add FAQ</h3>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $("#btnCreateFAQ").click(function () {
                var EF_event_id = $("#EF_event_id").val();
                var EF_question = $("#EF_question").val();
                var EF_answer = $("#EF_answer").val();
                if (EF_event_id === '0') {
                    $("#faqError").html('<div class="alert alert-danger fade in"><button class="close" type="button" data-dismiss="alert" aria-hidden="true">×</button><strong><em>Select event title</em></strong></div>');
                } else if (EF_question === "") {
