...
if (isset($_POST["btnCreateGallery"])) {
    extract($_POST);
    //alert sms show 
    if (empty($event_id) || empty($pay_id)) {
        $err = "Please select a event name and eventwise payment method";
    }

    if (!empty($event_id) && !empty($pay_id)) { //alert sms show data exit already
        $insert_iteam = "";
        $insert_iteam .= "event_id = '" . $event_id . "'";
        $insert_iteam .= ",payment_method_id = '" . $pay_id . "'";
        $insert_iteam .= ",date = '" . date('Y-m-d') . "'";
        $insert_iteam .= ",status = '1'";
        $sql_insert_iteam = "INSERT INTO eventwise_payment_method SET $insert_iteam ";
        $iteam_result = mysqli_query($con, $sql_insert_iteam);
       
       if ($iteam_result == 1) {
           $msg = "Successfully submit Data.";
            $link = "eventwise_payment_method_list.php?msg=" . base64_encode($msg);
            redirect($link);
        } else {
...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $evtwisepy = "SELECT * FROM  eventwise_payment_method WHERE id = '" . $id . "'";
    $eventpayarray = array();
...
if (isset($_POST["btnEditGallery"])) {
    ...
    if (!empty($event_id) && !empty($pay_id)) { //alert sms show data exit already
        ...
        $insert_iteam .= ",date = '" . date('Y-m-d') . "'";
        $insert_iteam .= ",status = '1'";
        $sql_insert_iteam = "UPDATE eventwise_payment_method SET $insert_iteam ";
        $iteam_result = mysqli_query($con, $sql_insert_iteam);
...
<html>
    ...
    <body class="">
        ...
        <div id="content">
            ...
            <h3 class="bg-white content-heading border-bottom strong">Edit Eventwise Payment Method </h3>
                        ...
                        <div class="widget-body">
                            <div class="row">
                                <div class="col-md-9">
                                    ...
                                    <div class="form-group">
                                        <label class="col-md-4 control-label" for="venueTitle">Eventwise Payment Method</label>
                                        <div class="col-md-8">
                                            <select class="form-control" id="paymet_id" name="pay_id">
                                                <option value="0">Select Payment method</option>
                                                <?php if (count($payarray) >= 1): ?>
                                                    <?php foreach ($payarray as $paymethod): ?>
                                                        <option <?php if ($eventpayarray[0]->payment_method_id == $paymethod->id) { ?> selected="selected" <?php } ?> value="<?php echo $paymethod->id; ?>"><?php echo $paymethod->name; ?></option>
                                                    <?php endforeach; ?>
                                                <?php endif; ?>
                                            </select>
                                        </div>
         ...
         <?php } else { ?>
            ...
            <h3 class="bg-white content-heading border-bottom strong "> Add Eventwise Payment Method </h3>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-8">
                                            <select class="form-control" id="paymet_id" name="pay_id">
                                                ...
                                                <?php if (count($payarray) >= 1): ?>
                                                    <?php foreach ($payarray as $paymethod): ?>
                                                        <option value="<?php echo $paymethod->id; ?>"><?php echo $paymethod->name; ?></option>
                                                    <?php endforeach; ?>
                                                <?php endif; ?>
                                            </select>
                                        </div>
...
<!--    <script>
    ...
    <script type="text/javascript">
        $("#eventwisepaymethod").addClass("active");
        $("#eventwisepaymethod").parent().parent().addClass("active");
        $("#eventwisepaymethod").parent().addClass("in");
    </script>
