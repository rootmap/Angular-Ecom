...
class configtoapi {
    ...
    public function open() {
        ...
        $con = mysqli_connect("localhost", "root", "", "ticketchai-ori");
        return $con;
    }
...
class GenerateKeySecret {
    ...
    function ApiAccessLink($param = '') {
        ...
        $credential = "password=tigger&trxid=" . $cur_random_value . "" . $conparam . "&format=xml";
...
class XmlToJson {
    ...
    public function getMovieName($index, $st = 0) {
        ...
        $api_link = "https://api.blockbusterbd.net/TestMode/MovieList.php?username=test@test.com&request_id=$movielistparam";
    ...
    public function getTheatreName($index, $st = 0) {
        ...
        $api_link = "https://api.blockbusterbd.net/TestMode/TheatreList.php?username=test@test.com&request_id=$theaterlistparam";
        //$dd = file_get_contents($api_link);
...
//        return $array;
    ...
    public function getShowTime($MovieID, $RequestDate) {
        ...
        $api_link = "https://api.blockbusterbd.net/TestMode/MovieSchedule.php?username=test@test.com&request_id=$movieinfoparam";
        //echo $dd = file_get_contents($api_link);
        $theatre = $this->Parse($api_link);
        $yummy = json_decode($theatre);
    ...
    public function getShowTimeSeatStatus($DTMID, $SLOT) {
        ...
        $api_link = "https://api.blockbusterbd.net/TestMode/MovieScheduleTheatreSeatStatus.php?username=test@test.com&request_id=$theatreseatstatusinfoparam";
        $theatre = $this->Parse($api_link);
        $yummy = json_decode($theatre);
    ...
    public function SecureBookingApi($DTMSID, $SeatClass, $Seat, $CusName, $CusEmail, $CusMobile) {
        ...
        $api_link = "https://api.blockbusterbd.net/TestMode/MovieSeatBookingRequest.php?username=test@test.com&request_id=$theatreseatstatusinfoparam";
        $theatre = $this->Parse($api_link);
        $yummy = json_decode($theatre);
        if($yummy->status=="9999")
        {
    ...
    public function SecureBookingConfirm($DTMSID, $LID) {
        $secure = new GenerateKeySecret();
        $booking_param = array("DTMSID" => $DTMSID, "LID" => $LID, "ConfirmStatus" => "CONFIRM");
        $theatreseatstatusinfoparam = $secure->ApiAccessLinkConfirm($booking_param, "1453182256");
        $api_link = "https://api.blockbusterbd.net/TestMode/MovieSeatUpdateStatus.php?username=test@test.com&request_id=$theatreseatstatusinfoparam";
        $theatre = $this->Parse($api_link);
        $yummy = json_decode($theatre);
        $array = array("trx_id" => $yummy->trx_id,
            "lid" => $yummy->lid,
    ...
    public function SecureBookingCancel($DTMSID, $LID) {
        $secure = new GenerateKeySecret();
    ...
    public function SecureBalanceStatement() {
        ...
        $api_link = "https://api.blockbusterbd.net/TestMode/balance.php?username=test@test.com&request_id=$theatreseatstatusinfoparam";
        $theatre = $this->Parse($api_link);
        $yummy = json_decode($theatre);
